...
def strip_chapter_title_heading(text: str, section: str, chapter: str) -> str:
    """
    Strips leading section and chapter header titles from the start of the chapter body text
    so that chapter_content contains purely the narrative content.
    """
    cleaned = text

    # 1. Strip known book section titles at the beginning (e.g. 'THE WISDOM OF LIFE.', 'ESSAYS.')
    cleaned = re.sub(r'^(?:THE\s+WISDOM\s+OF\s+LIFE|ESSAYS)\.?\s*', '', cleaned, flags=re.IGNORECASE)

    # 2. Strip Chapter labels (e.g. 'CHAPTER I.', 'CHAPTER IV.')
    cleaned = re.sub(r'^CHAPTER\s+[IVXLCDM\d]+[\.:]?\s*', '', cleaned, flags=re.IGNORECASE)

    # 3. Strip Chapter titles (handling multi-line, hyphens, OCR variations like 'SU I CIDE', trailing asterisks/punctuation)
    ...
    # 4. Strip secondary section/subtitles at chapter start (e.g. 'SECTION I. REPUTATION.', 'Section i.\n\nON THE SAME.')
...
def aggregate_documents_by_chapter(documents):
    ...
    for doc_group in chapter_groups:
        ...
        # Strip chapter & section title headers from the start of the chapter content
        cleaned_chapter_text = strip_chapter_title_heading(cleaned_chapter_text, section, chapter)
